...
def one_height_slice(mesh, begin_height, box_size,box_height, vectors=None, fraction=3):
    if vectors is None:
    ...
    slice_plane, vectors = one_slice(vectors, height, x_min, x_max, y_min, y_max, box_size)
    # print(f'vectors len {len(vectors)}')
    # print("Temp slice on height %.3f made" % height)
    for iter in range(fraction - 2):
        height += step
        t_slice_plane,vectors = one_slice(vectors, height, x_min, x_max, y_min, y_max, box_size)
        # print(f'vectors len {len(vectors)}')
        slice_plane +=t_slice_plane
        # print("Temp slice on height %.3f made" % height)

    t_slice_plane, vectors = one_slice(vectors, begin_height + box_height, x_min, x_max, y_min, y_max, box_size)
    # print(f'vectors len {len(vectors)}')
    slice_plane += t_slice_plane
...
def make_slice(mesh, box_size,box_height, fraction=3):
    ...
    for i in range(z_size):
        ...
        # print(f'G vectors len {len(vectors) if vectors is not None else -1}')
        temp_slice,vectors = one_height_slice(mesh, i * box_height, box_size, box_height, vectors=vectors, fraction=fraction)
        sliced_image[:, :, i] = temp_slice
        current_height = i * box_height + box_height / 2
